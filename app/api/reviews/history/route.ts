 ...
 * Fetches the user's review history
 * 
 * Headers:
 * - Authorization: Bearer {token} (required)
 * 
 * Backend: GET /api/v2/reviews/history
 */
export async function GET(req: NextRequest) {
  try {
    ...
    // Call backend API: GET /api/v2/reviews/history
    ...
    return NextResponse.json(
      { success: false, message: "Failed to load review history" },
      { status: 500 }
    );
  }
}
