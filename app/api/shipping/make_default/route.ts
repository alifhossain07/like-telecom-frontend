...
/**
 * POST /api/shipping/make_default
 * Makes a shipping address the default address
 * 
 * Headers:
 ...
 * 
 * Backend: POST /api/v2/user/shipping/make_default
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Call backend API
    const res = await fetch(`${apiBase}/user/shipping/make_default`, {
      method: "POST",
      headers: {
    ...
    console.error("Shipping make default error:", error);
    return NextResponse.json(
      { success: false, message: "Failed to set default shipping address" },
      { status: 500 }
    );
  }
}
