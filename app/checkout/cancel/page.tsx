            ...
            <div className="w-full max-w-2xl border rounded-2xl p-6 md:p-12 bg-white shadow-sm text-center">
                ...
                {!loading && (
                    <>
                        ...
                        {/* Order Brief Info */}
                        <div className="w-full bg-amber-50 rounded-2xl border border-amber-100 overflow-hidden text-left mb-8">
                            <div className="p-5">
                                <h3 className="text-xs font-bold uppercase tracking-wider text-amber-800/60 mb-4 border-b border-amber-100 pb-2">Order Summary</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <p className="text-[10px] uppercase text-amber-400 font-bold">Order ID</p>
                                        <p className="text-sm font-semibold text-gray-800">{orderIdState || "N/A"}</p>
                                    </div>
                                    {totals && (
                                        <div>
                                            <p className="text-[10px] uppercase text-amber-400 font-bold">Total Amount</p>
                                            <p className="text-sm font-semibold text-gray-800">৳{formatPrice(totals.total)}</p>
                                        </div>
                                    )}
                                    {customer && (
                                        <div className="col-span-1 md:col-span-2">
                                            <p className="text-[10px] uppercase text-amber-400 font-bold">Customer</p>
